    ...
    def other_tensors(self):
        ...
        for i in range(self.args.num_hiddens):
            ...
            # train with inversed auxiliary loss.
                r=0.5
                self.train_ops.append(tf.train.AdamOptimizer(self.learning_rate*0.1).minimize(tf.reduce_sum([self.loss_list[ii]*(r**(ii+1)) for ii in range(i,self.args.num_hiddens)])/
...
# ==========================================================
    def inference(self, input_img, depth, scale):
...
# ==========================================================
    def test_plot(self, infer_image):
        ...
        for k in range(5):
            ...
            for i in range(self.args.num_hiddens):
                start_time = time.time()
                result = self.inference(infer_image_downscaled, depth=i, scale= self.args.scale)
                elapse = time.time() - start_time
